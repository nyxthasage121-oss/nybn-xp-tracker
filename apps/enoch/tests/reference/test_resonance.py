"""Tests for Enoch/reference/resonance.py."""
...
def test_get_temperament_negligible():
    ...
    for die_value in range(1, 6):
        with patch("Enoch.d10", return_value=die_value):
...
def test_get_temperament_intense():
    ...
    for first_die in [9, 10]:
        for second_die in range(1, 9):
            with patch("Enoch.d10", side_effect=[first_die, second_die]):
                result = _get_temperament()
...
def test_get_resonance(mode, die_value, expected_resonance):
    ...
    with patch("Enoch.random", return_value=die_value):
...
def test_get_resonance_standard_mode_cap():
    ...
    with patch("Enoch.random", return_value=5) as mock_random:
...
async def test_random_temperament():
    ...
    with (
        ...
        patch("Enoch.reference.resonance.get_avatar", return_value="http://avatar.url"),
    ):
        mock_mode.return_value = ResonanceMode.STANDARD

        # Mock the temperament generation
        with patch("Enoch.d10", return_value=7):
...
async def test_random_temperament_negligible():
    ...
    with (
        ...
        with patch("Enoch.d10", return_value=3):
            await random_temperament(mock_ctx, "Choleric")
...
async def test_resonance_with_temperament():
    ...
    with (
        ...
        with patch("Enoch.d10", return_value=7), patch("Enoch.random", return_value=7):
...
async def test_display_embed_standard_mode():
    ...
    # Mock get_avatar
    with patch("Enoch.reference.resonance.get_avatar", return_value="http://avatar.url"):
...
async def test_display_embed_acute_with_dyscrasia():
    ...
    with (
        patch("Enoch.reference.resonance.get_avatar", return_value="http://avatar.url"),
        patch("Enoch.reference.resonance.get_dyscrasia", return_value=mock_dyscrasia),
    ):
        await _display_embed(
            mock_ctx,
            temperament="Acute",
...
async def test_display_embed_acute_without_dyscrasia():
    ...
    with (
        patch("Enoch.reference.resonance.get_avatar", return_value="http://avatar.url"),
        patch("Enoch.reference.resonance.get_dyscrasia", return_value=None),
    ):
        await _display_embed(
            mock_ctx,
            temperament="Acute",
