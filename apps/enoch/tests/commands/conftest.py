...
def bot() -> EnochBot:
    ...
    # Mock the bot user for tests that access Enoch.bot.user
    bot_user = MagicMock()
    bot_user.display_name = "Enoch"
...
async def mock_character_display() -> AsyncGenerator[AsyncMock, None]:
    ...
    with patch("Enoch.character.display", new_callable=AsyncMock) as mock:
        yield mock
...
@pytest.fixture(autouse=True)
def mock_enoch_bot(bot: EnochBot):
    """Patch the global Enoch.bot with our test bot."""
    with patch("Enoch.bot", bot):
        yield bot
