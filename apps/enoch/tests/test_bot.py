...
@pytest.fixture
def mock_bot():
    """Create a mock EnochBot instance."""
    bot = MagicMock(spec=EnochBot)
    bot.guilds = []
    return bot
...
async def test_on_guild_join_before_ready(mock_bot):
    ...
    with (
        patch("Enoch.stats.guild_joined", new_callable=AsyncMock),
        patch.object(mock_bot, "_set_presence", new_callable=AsyncMock),
    ):
        await EnochBot.on_guild_join(mock_bot, guild)
...
async def test_on_guild_remove_before_ready(mock_bot):
    ...
    with (
        patch("Enoch.stats.guild_left", new_callable=AsyncMock),
        patch.object(mock_bot, "_set_presence", new_callable=AsyncMock),
    ):
        await EnochBot.on_guild_remove(mock_bot, guild)
...
async def test_on_guild_update_before_ready(mock_bot):
    ...
    with patch("Enoch.stats.guild_renamed", new_callable=AsyncMock):
        await EnochBot.on_guild_update(guild_before, guild_after)
...
async def test_on_member_join_before_ready(mock_bot):
    ...
    with patch("services.char_mgr.mark_active", new_callable=AsyncMock):
        await EnochBot.on_member_join(member)
...
async def test_on_member_remove_before_ready(mock_bot):
    ...
    with patch("services.char_mgr.mark_inactive", new_callable=AsyncMock):
        await EnochBot.on_member_remove(member)
...
async def test_on_member_update_before_ready(mock_bot):
    ...
    await EnochBot.on_member_update(mock_bot, member_before, member_after)
