...
class EnochBot(discord.AutoShardedBot):
    ...
    async def inform_premium_loss(member: discord.Member, title="You are no longer a supporter!"):
        ...
        try:
            server = f"[Enoch server]({constants.SUPPORT_URL})"
        ...
        try:
            ...
            embed = discord.Embed(
                ...
                description=(
                    ...
                    "[Read more here!](https://docs.Enoch.app/guides/premium/character-images)"
                ),
    ...
    async def prep_webhook(self, channel: discord.TextChannel) -> discord.Webhook:
        ...
        try:
            ...
            raise errors.WebhookError("Enoch needs `Manage Webhook` permission for this command.")
    ...
    async def on_guild_join(self, guild: discord.Guild):
        ...
        await Enoch.stats.guild_joined(guild)
        await self._set_presence()
        if await services.guild_cache.ready():
    ...
    async def on_guild_remove(self, guild: discord.Guild):
        ...
        await Enoch.stats.guild_left(guild)
        await self._set_presence()
        if await services.guild_cache.ready():
    ...
    async def on_guild_update(before: discord.Guild, after: discord.Guild):
        ...
        if before.name != after.name:
            ...
            await Enoch.stats.guild_renamed(after, after.name)
...
bot = EnochBot(intents=intents, debug_guilds=DEBUG_GUILDS, cache_app_emojis=True)
Enoch.bot = bot
