...
class VChar(BaseModel):
    ...
    def pre_insert(self):
        """Last-minute prep before first database insert."""
    ...
    def _to_row_params(self) -> dict:
        """Serialize fields to a dict suitable for SQL parameter binding."""
        return {
            "guild": self.guild,
            "user": self.user,
            "name": self.raw_name,
            ...
            "humanity": self.raw_humanity,
            "stains": self.stains,
            "hunger": self.raw_hunger,
            "potency": self.potency,
            "traits": json.dumps([t.model_dump(by_alias=True) for t in self.raw_traits]),
            "profile": self.profile.model_dump_json(),
            "convictions": json.dumps(self.convictions),
            "header": self.header.model_dump_json(),
            "macros": json.dumps([m.model_dump() for m in self.macros]),
            "experience": self.experience.model_dump_json(by_alias=True),
            "stat_log": json.dumps(self.stat_log, default=str),
        }

    @classmethod
    def from_row(cls, row) -> "VChar":
        ...
        obj = cls.model_validate({
            ...
            "convictions": json.loads(row["convictions"] or "[]"),
            "header": json.loads(row["header"] or "{}"),
            "macros": json.loads(row["macros"] or "[]"),
            "experience": json.loads(row["experience"] or "{}"),
            "log": json.loads(row["stat_log"] or "{}"),
        })
        obj.id = row["id"]
        return obj

    @classmethod
    async def find_all(cls) -> list["VChar"]:
        """Fetch all characters from the database."""
        async with get_engine().connect() as conn:
            result = await conn.execute(text("SELECT * FROM inconnu_characters"))
            rows = result.mappings().all()
        return [cls.from_row(row) for row in rows]

    async def insert(self):
        """Insert this character into the database and set self.id."""
        self.pre_insert()
        async with get_engine().begin() as conn:
            result = await conn.execute(
                text("""
                    INSERT INTO inconnu_characters
                        (guild, user, name, splat, health, willpower, humanity, stains,
                         hunger, potency, traits, profile, convictions, header,
                         macros, experience, stat_log)
                    VALUES
                        (:guild, :user, :name, :splat, :health, :willpower, :humanity, :stains,
                         :hunger, :potency, :traits, :profile, :convictions, :header,
                         :macros, :experience, :stat_log)
    ...
    async def save(self):
        ...
        async with get_engine().begin() as conn:
            await conn.execute(
                text("""
                    UPDATE inconnu_characters
                    SET guild=:guild, user=:user, name=:name, splat=:splat,
                        health=:health, willpower=:willpower, humanity=:humanity,
                        stains=:stains, hunger=:hunger, potency=:potency,
                        traits=:traits, profile=:profile, convictions=:convictions,
                        header=:header, macros=:macros, experience=:experience,
                        stat_log=:stat_log
                    WHERE id=:id
                """),
                params,
    ...
    async def delete(self) -> bool:
        """Delete this character from the database. Returns True if deleted."""
        async with get_engine().begin() as conn:
            result = await conn.execute(
                text("DELETE FROM inconnu_characters WHERE id=:id"),
                {"id": self.id},
            )
