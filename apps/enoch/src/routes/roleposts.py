...
async def get_rolepost(
    oid: int, _: HTTPAuthorizationCredentials = Depends(verify_api_key)
):
    """Fetch Rolepost data."""
    rolepost = await RPPost.find_by_id(oid)
    if rolepost is None:
        raise HTTPException(404, detail="Rolepost not found")

    guild = await Enoch.bot.get_or_fetch_guild(rolepost.guild)
    if guild is None:
        raise HTTPException(410, detail="Enoch is not in this guild.")
