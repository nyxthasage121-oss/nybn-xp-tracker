...
class ErrorReporter:
    ...
    def reinvite_message(self) -> discord.Embed:
        ...
        embed = discord.Embed(
            title="Please re-invite Enoch",
            description=(
                f"**Enoch** has encountered an error. Please [re-invite]({invite}) "
    ...
    async def report_error(self, ctx: discord.ApplicationContext | discord.Interaction, error):
        ...
        if isinstance(error, errors.NotPremium):
            troubleshoot_url = (
                "https://docs.Enoch.app/advanced/troubleshooting#you-arent-able-to-upload-images"
            )
            ...
            await ui.embeds.error(
                ...
                help="https://docs.Enoch.app/guides/premium",
            )
            return
