...
class Gameplay(commands.Cog):
    ...
    async def vr(
        ...
        await Enoch.vr.parse(ctx, syntax, comment, character, player)
    ...
    async def easy_roll(
        ...
        char = await Enoch.vr.parse(ctx, syntax, comment, character, player)
    ...
    async def aggheal(
        ...
        await Enoch.misc.aggheal(ctx, character)
    ...
    async def awaken(
        ...
        await Enoch.misc.awaken(ctx, character)
    ...
    @Enoch.options.char_option("The character to Blush")
    async def bol(
        self,
        ctx: discord.ApplicationContext,
        ministry_alt: bool,
        ...
        await Enoch.misc.bol(ctx, character, ministry_alt)

    @slash_command(contexts={discord.InteractionContextType.guild})
    @option(
        "difficulty", description="The frenzy difficulty", choices=Enoch.options.ratings(1, 10)
    )
    @option(
        "penalty",
    ...
    async def frenzy(
        ...
        await Enoch.misc.frenzy(ctx, character, difficulty, penalty, bonus)
    ...
    async def remorse(
        ...
        await Enoch.misc.remorse(ctx, character, min_override, lasombra_alt)
    ...
    async def rouse(
        ...
        await Enoch.misc.rouse(ctx, character, count, purpose, bool(reroll))

    @slash_command(contexts={discord.InteractionContextType.guild})
    @option("amount", description="amount", choices=Enoch.options.ratings(1, 5))
    ...
    async def slake(
        ...
        await Enoch.misc.slake(ctx, character, amount)
    ...
    async def stain(
        ...
        await Enoch.misc.stain(ctx, character, delta, player=player)
