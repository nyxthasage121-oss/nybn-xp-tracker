        ...
        if show_support:
            self.add_item(
                Button(
                    ...
                    url="https://docs.Enoch.app/guides/quickstart",
                    row=1,
                )
            )
...
class Help(commands.Cog):
    ...
    async def help_command(self, ctx: discord.ApplicationContext, section: str):
        ...
        match section:
            ...
            case Section.CHARACTERS:
                await Enoch.character.update_help(ctx, ephemeral=False)
            case Section.TRAITS:
    ...
    async def info(self, ctx):
        ...
        embed = discord.Embed(
            ...
            description="**Enoch** is a dice roller for Vampire: The Masquerade 5th Edition.",
        )
        ...
        embed.add_field(
            name="Source code", value="[GitHub](https://github.com/tiltowait/Enoch)", inline=False
        )
    ...
    async def show_traits_help(self, ctx, ephemeral=True):
        ...
        buttons = [
            Button(label="Documentation", url="https://docs.Enoch.app/command-reference/traits"),
    ...
    async def show_specialties_help(self, ctx, ephemeral=True):
        ...
        buttons = [
            Button(
                label="Documentation",
                url="https://docs.Enoch.app/guides/quickstart/specialties",
            ),
            Button(label="Server", url=SUPPORT_URL),
        ]
        view = _HelpView(*buttons)
    ...
    async def show_disciplines_help(self, ctx, ephemeral=True):
        ...
        buttons = [
            Button(
                label="Documentation",
                url="https://docs.Enoch.app/guides/quickstart/disciplines-and-powers",
            ),
            Button(label="Server", url=SUPPORT_URL),
        ]
        view = _HelpView(*buttons)
    ...
    async def show_macros_help(self, ctx, ephemeral=True):
        ...
        buttons = [
            Button(label="Documentation", url="https://docs.Enoch.app/command-reference/macros"),
            Button(label="Server", url=SUPPORT_URL),
        ]
        view = _HelpView(*buttons)
    ...
    async def show_character_help(self, ctx, ephemeral=True):
        """Run the /macro help command."""
        await Enoch.character.update_help(ctx, ephemeral=ephemeral)
    ...
    async def show_rp_posts_help(self, ctx, ephemeral=True):
        ...
        embed = self._empty_embed(
            title="Roleposting",
            description=(
                "This feature is for [premium users only](https://docs.Enoch.app/guides/premium)."
            ),
        )
        embed.add_field(
            name="WARNING!",
            value=(
                ...
                "`/invite` to re-invite **Enoch** with this permission "
                "enabled."
            ),
            inline=False,
        )
        ...
        buttons = [
            Button(label="Documentation", url="https://docs.Enoch.app/premium/roleposting"),
            Button(label="Server", url=SUPPORT_URL),
        ]
        view = _HelpView(*buttons)
