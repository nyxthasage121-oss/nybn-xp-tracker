...
class ReferenceCommands(commands.Cog):
    ...
    @slash_command()
    @option(
        "rating", description="The Blood Potency rating", choices=Enoch.options.ratings(1, 10)
    )
    async def bp(
        self,
        ...
        await Enoch.reference.blood_potency(ctx, rating)
    ...
    async def cripple(
        ...
        await Enoch.reference.cripple(ctx, damage)
    ...
    async def probability(
        ...
        await Enoch.reference.probability(ctx, roll, reroll, character)

    @slash_command()
    async def resonance(self, ctx):
        """Generate a random Resonance."""
        await Enoch.reference.resonance(ctx)
    ...
    async def statistics(
        ...
        await Enoch.reference.statistics(ctx, character, style, date, player=player)

    @slash_command()
    @option(
        "resonance",
        description="The resonance being rolled",
        choices=Enoch.reference.STANDARD_RESONANCES,
    )
    async def temperament(
        ...
        await Enoch.reference.random_temperament(ctx, resonance)
    ...
    async def toggle_roll_statistics(self, ctx, message: discord.Message):
        ...
        if ctx.bot.user != message.author:
            await ctx.respond("This isn't an Enoch roll message.", ephemeral=True)
            return

        toggled = await Enoch.stats.toggle_roll_stats(message.id)
        if toggled is None:
            await ctx.respond(
    ...
    async def on_raw_bulk_message_delete(self, payload):
        ...
        if deletions:
            ...
            await Enoch.stats.roll_message_deleted(*deletions)
    ...
    async def on_raw_message_delete(self, raw_message):
        ...
        async def deletion_handler(message_id: int):
            ...
            await Enoch.stats.roll_message_deleted(message_id)
    ...
    async def on_guild_channel_delete(self, channel):
        ...
        await Enoch.stats.delete_rolls_in_channel(channel)
