...
import services
import ui
from enoch.options import char_option
...
class MiscCommands(commands.Cog):
    ...
    async def changelog(self, ctx, hidden: bool):
        """Show Enoch's most recent changelog."""
        await Enoch.misc.show_changelog(ctx, hidden)

    @slash_command()
    async def coinflip(self, ctx):
        """Flip a coin."""
        await Enoch.misc.coinflip(ctx)

    @slash_command()
    async def invite(self, ctx):
        """Display Enoch's invite link."""
        embed = discord.Embed(
            title="Invite Enoch to your server",
            url=ctx.bot.invite_url,
            description="Click the link above to invite Enoch to your server!",
        )
        embed.set_author(name=ctx.bot.user.display_name)
        embed.set_thumbnail(url=ctx.bot.user.display_avatar)
        site = discord.ui.Button(label="Website", url="https://www.Enoch.app")
    ...
    async def random(
        ...
        await Enoch.misc.percentile(ctx, ceiling)
