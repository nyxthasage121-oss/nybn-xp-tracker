    ...
    async def respond_to_button(self, btn):
        ...
        if button_id == _ButtonID.WILLPOWER:
            ...
            await Enoch.character.display(
                ...
                owner=self.owner,
                fields=[("New WP", Enoch.character.DisplayField.WILLPOWER)],
            )
            ...
            if button_id == self.character.id_str:
                ...
                self.surged = True
                await Enoch.misc.rouse(btn, self.character, 1, "Surge", False)

            elif button_id == self.character.id_str + "-rr":
                # Fluent X
                self.surged = True
                await Enoch.misc.rouse(btn, self.character, 1, "Surge", True)
