...
CHANGELOG = "https://github.com/tiltowait/Enoch/releases/latest"


async def show_changelog(ctx: discord.ApplicationContext, hidden: bool):
    """Display Enoch's changelog."""
    try:
        ...
        for page in paginator.pages:
            embed = discord.Embed(title=f"Enoch {tag}", description=page, url=CHANGELOG)
...
async def fetch_changelog() -> tuple[str, str]:
    ...
    async with aiohttp.ClientSession(headers=header, raise_for_status=True) as session:
        async with session.get(
            "https://api.github.com/repos/tiltowait/Enoch/releases/latest"
        ) as res:
            json = await res.json()
