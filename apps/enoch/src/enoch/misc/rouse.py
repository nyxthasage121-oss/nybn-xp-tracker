...
__HELP_URL = "https://docs.Enoch.app/guides/gameplay-shortcuts#rouse-checks"
...
async def __display_outcome(ctx, character: VChar, outcome, purpose, oblivion, message):
    ...
    footer = []
    fields = [
        ("New Hunger" if "ailure" in title else "Hunger", Enoch.character.DisplayField.HUNGER)
    ]
    ...
    if outcome.stains > 0:
        ...
        if oblivion == "show":
            ...
            fields.append((f"Gain {stains_txt}", Enoch.character.DisplayField.HUMANITY))

    footer = "\n".join(footer)

    view = ui.views.FrenzyView(character, 4, ctx.user.id) if outcome.frenzy else None

    return await Enoch.character.display(
        ctx,
        character,
        title=title,
        footer=footer,
