...
class HandledError(EnochError):
    """An error that needs no further handling."""


class RollError(EnochError):
    """Base error pertaining to rolls."""


class WebhookError(EnochError):
...
class CharacterError(EnochError):
...
class MacroNotFoundError(EnochError):
    """Error for when a macro isn't found."""


class InvalidLogKeyError(EnochError):
