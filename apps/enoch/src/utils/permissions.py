...
def is_storyteller(ctx: AppInteraction) -> bool:
    """Check if the user has the configured Storyteller role."""
    if STORYTELLER_ROLE is None:
        return False
    user = cast(discord.Member, ctx.user)
    return user.get_role(STORYTELLER_ROLE) is not None


def require_admin_or_storyteller():
    """A @commands.check decorator that passes for server admins and Storytellers."""
